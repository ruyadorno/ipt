...
function iPipeTo(input, {stdin = process.stdin, stdout = process.stdout, sep = require('os').EOL, ...options}, __prompt) {
	...
	return result
		...
		.then(answers => options.copy ? clipboard(answers.join(sep)).then(() => answers) : answers);
}
